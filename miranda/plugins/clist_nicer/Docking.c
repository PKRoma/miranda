...
int Docking_ProcessWindowMessage(WPARAM wParam,LPARAM lParam)
{
	...
	if(msg->message==WM_DESTROY) DBWriteContactSettingByte(NULL,"CList","Docked",(BYTE)docked);
	if(!docked && msg->message!=WM_CREATE && msg->message!=WM_MOVING && msg->message!=WM_CREATEDOCKED && msg->message != WM_MOVE) return 0;
	switch(msg->message) {
		case WM_CREATE:
			//if(GetSystemMetrics(SM_CMONITORS)>1) return 0;
			if(DBGetContactSettingByte(NULL,"CList","Docked",0)) PostMessage(msg->hwnd,WM_CREATEDOCKED,0,0);
			draggingTitle=0;
			return 0;
