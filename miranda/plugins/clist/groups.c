...
static int CreateGroup(WPARAM wParam, LPARAM lParam)
{
	...
	if (wParam) {
		...
		_sntprintf( newBaseName, sizeof(newBaseName), _T("%s\\%s"), dbv.pszVal + 1, TranslateT("New Group"));
		newBaseName[ sizeof(newBaseName)-1 ] = 0;
		mir_free(dbv.pszVal);
	}
	...
	while (GroupNameExists(newName + 1, -1)) {
		_sntprintf(newName + 1, sizeof(newName) - 1, _T("%s (%d)"), newBaseName, ++i);
...
static int DeleteGroup(WPARAM wParam, LPARAM lParam)
{
	...
	if (DBGetContactSettingByte(NULL, "CList", "ConfirmDelete", SETTING_CONFIRMDELETE_DEFAULT))
		if (MessageBoxA((HWND)CallService(MS_CLUI_GETHWND, 0, 0), Translate("Are you sure you want to delete this group?  This operation can not be undone."), Translate("Delete Group"), MB_YESNO|MB_ICONQUESTION)==IDNO)
			return 1;
	lstrcpynA(name, dbv.pszVal + 1, sizeof(name));
	DBFreeVariant(&dbv);
		...
		for (i = 0;; i++) {
			...
			if (!strncmp(dbv.pszVal + 1, name, len) && dbv.pszVal[len + 1] == '\\' && strchr(dbv.pszVal + len + 2, '\\') == NULL) {
				if (szNewParent[0])
					...
					lstrcpynA(szNewName, dbv.pszVal + len + 2, sizeof(szNewName));
...
static int RenameGroupWithMove(int groupId, const TCHAR *szName, int move)
{
	...
	if (GroupNameExists(szName, groupId)) {
		MessageBoxA(NULL, Translate("You already have a group with that name. Please enter a unique name for the group."), Translate("Rename Group"),
			MB_OK);
		return 1;
	}
	...
	//rename subgroups
	{
		...
		for (i = 0;; i++) {
			...
			if ( !_tcsncmp(dbv.ptszVal + 1, oldName, len) && dbv.ptszVal[len + 1] == '\\' && _tcschr(dbv.ptszVal + len + 2, '\\') == NULL) {
				_sntprintf(szNewName, sizeof(szNewName), _T("%s\\%s"), szName, dbv.pszVal + len + 2);
	...
	for (groupId = 0;; groupId++) {
		...
		do {
			...
			for (menuId = 0; menuId < menuItemCount; menuId++) {
				...
				GetMenuItemInfoA(hThisMenu, menuId, TRUE, &mii);
				compareResult = lstrcmpA(szThisField, szThisMenuItem);
				if (compareResult == 0) {
					if (pNextField == NULL) {
						mii.fMask = MIIM_DATA;
						mii.dwItemData = groupId + 1;
						SetMenuItemInfoA(hThisMenu, menuId, TRUE, &mii);
					}
					else {
						if (mii.hSubMenu == NULL) {
							...
							mii.wID = nextMenuId++;
							InsertMenuItemA(mii.hSubMenu, 0, TRUE, &mii);
							mii.fMask = MIIM_TYPE;
							mii.fType = MFT_SEPARATOR;
							InsertMenuItemA(mii.hSubMenu, 1, TRUE, &mii);
						}
			...
			if (compareResult) {
				...
				InsertMenuItemA(hThisMenu, menuId, TRUE, &mii);
				if (pNextField) {
					hThisMenu = mii.hSubMenu;
				}
			}
