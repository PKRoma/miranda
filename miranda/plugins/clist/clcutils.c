                ...
                if (contact->type == CLCIT_GROUP && !strstr(text, "\\")) {
                    ...
                    if (text[0] == '\0') {
                        ...
                        if (!lstrcmp(otherName, text))
                            ...
                            DBWriteContactSettingString(contact->hContact, "CList", "MyHandle", text);
                    }
                    if (otherName)
                        mir_free(otherName);
                }
...
void BeginRenameSelection(HWND hwnd, struct ClcData *dat)
{
    ...
    dat->hwndRenameEdit =
        CreateWindow("EDIT", contact->szText, WS_CHILD | WS_BORDER | ES_AUTOHSCROLL, x, y, clRect.right - x, dat->rowHeight, hwnd, NULL, g_hInst,
                     NULL);
