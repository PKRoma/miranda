			...
			if (flags != (DWORD) - 1 && pNextField == NULL && calcTotalMembers) {
				...
				while (hContact) {
					if (!DBGetContactSettingTString(hContact, "CList", "Group", &dbv)) {
						if (!lstrcmp(dbv.ptszVal, szName) && (style & CLS_SHOWHIDDEN || !DBGetContactSettingByte(hContact, "CList", "Hidden", 0)))
							group->totalMembers++;
	...
	if (!FindItem(hwnd, dat, hItem, &contact, &group, NULL)) {
		...
		for (i = 0;; i++) {
			...
			if (group->contact[i].type == CLCIT_GROUP) {
				...
				if (!_tcsncmp(group->contact[i].szText, dbv.ptszVal + nameOffset, len) && 
