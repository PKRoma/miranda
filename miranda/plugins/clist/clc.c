...
static LRESULT CALLBACK ContactListControlWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case SB_LINEDOWN:
		...
		break;
	case SB_PAGEUP:
		desty -= clRect.bottom - dat->rowHeight;
		break;
	case SB_PAGEDOWN:
		desty += clRect.bottom - dat->rowHeight;
		break;
	...
	case VK_F2:
		...
		return 0;
	case VK_DELETE:
		DeleteFromContactList(hwnd, dat);
		return 0;
	default:
		{
			NMKEY nmkey;
	...
	case WM_LBUTTONDOWN:
		{
			...
			if (hit != -1 && contact->type == CLCIT_GROUP)
				if (hitFlags & CLCHT_ONITEMICON) {
					...
					if (dat->selection != -1) {
						dat->selection =
							GetRowsPriorTo(&dat->list, selgroup, ((unsigned) selcontact - (unsigned) selgroup->contact) / sizeof(struct ClcContact));
						if (dat->selection == -1)
