...
static void CLCPaint_OldInternalPaintRowItems(HWND hwnd, HDC hdcMem, struct ClcData *dat, struct ClcContact *Drawing,RECT row_rc, RECT free_row_rc, int left_pos, int right_pos, int selected,int hottrack, RECT *rcPaint)
{
	int item=0,
		item_iterator=0, 
		item_text=0;
			...
			case CIT_AVATAR:
				{
					...
					if ( dat->avatars_draw_border )
					{
						...
						CombineRgn(rgnOutside,rgnInside,rgnOutside,RGN_DIFF);
						FillRgn(hdcMem,rgnOutside,hBrush);
						SkinEngine_SetRgnOpaque(hdcMem,rgnOutside);
						SelectObject(hdcMem, hOldBrush);
						DeleteObject(hBrush);
			...
			case CIT_TEXT:
				{
					...
					if ( Drawing->type == CLCIT_GROUP )
					{
						SkinEngine_DrawText( hdcMem, Drawing->szText, -1,rc, uTextFormat);
						if ( selected && dat->szQuickSearch[0]!='\0' )
						{
							SetTextColor(hdcMem, dat->quickSearchColour);
							SkinEngine_DrawText( hdcMem, Drawing->szText, lstrlen(dat->szQuickSearch),rc, uTextFormat);
					...
					if ( Drawing->type == CLCIT_GROUP )
					{
						...
						if(szCounts && szCounts[0])
						{
							...
							if (selected)
								...
								CLCPaint_SetHotTrackColour(hdcMem,dat);
							SkinEngine_DrawTextA( hdcMem, szCounts, -1,rc, uTextFormat);
						}						
					}
					else if ( Drawing->type == CLCIT_CONTACT )
					{
