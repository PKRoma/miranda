...
__declspec(dllexport) PLUGININFOEX *MirandaPluginInfoEx(DWORD mirandaVersion)
{
	...
	if (mirandaVersion < PLUGIN_MAKE_VERSION(0, 8, 0, 0) || mirandaVersion >= PLUGIN_MAKE_VERSION(0, 9, 0, 0)) {
		MessageBox(0, _T("The installed version of the tabSRMM plugin is not compatible with this version of Miranda. It requires Miranda 0.8.x and cannot be loaded."), _T("tabSRMM error"), MB_OK | MB_ICONERROR);
		return NULL;
	}
	return &pluginInfo;
}
