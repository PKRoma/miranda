...
static VOID CALLBACK IconFlashTimer(HWND hwnd, UINT message, UINT idEvent, DWORD dwTime)
{
	...
	if (eventCount) {
		...
		TrayIconUpdateWithImageList((iconsOn
	...
	for (i = 0; i < eventCount; i++) {
		...
		if (j < i)
			continue;
		ChangeContactIcon(event[i].cle.hContact, iconsOn || disableIconFlash ? event[i].imlIconIndex : 0, 0);
...
static int RemoveEvent(WPARAM wParam, LPARAM lParam)
{
	...
	ChangeContactIcon(event[i].cle.hContact,
		ExtIconFromStatusMode( event[i].cle.hContact, szProto, 
		szProto == NULL ? ID_STATUS_OFFLINE : DBGetContactSettingWord(event[i].cle.hContact, szProto, "Status",
		ID_STATUS_OFFLINE)),0); //by FYR
