	...
	*pszGroup = '\0';
	if(!DBGetContactSetting(NULL, "Chat", "AddToGroup", &dbv) && dbv.type == DBVT_ASCIIZ)
	{
		if(lstrlen(dbv.pszVal) > 0)
			lstrcpyn(pszGroup, dbv.pszVal, 50);

		DBFreeVariant(&dbv);
	}
	else
		lstrcpyn(pszGroup, "Chat rooms", 50);
		...
		if(pszGroup && lstrlen(pszGroup) > 0)
		{
			for (i = 0;; i++) 
			{
				...
				if(dbv.type == DBVT_ASCIIZ)
				{
					...
					{
						if (dbv.pszVal[0] != '\0' && dbv2.pszVal[0] != '\0'&& !lstrcmpi(dbv.pszVal + 1, dbv2.pszVal))
						{
							DBFreeVariant(&dbv);
							DBFreeVariant(&dbv2);
							goto END_GROUPLOOP;
...
int	CList_RoomDoubleclicked(WPARAM wParam,LPARAM lParam)
{
	...
	if (MM_FindModule(szProto)) 
	{
		...
		{
			hWindow = WM_FindWindow(dbv.pszVal, szProto);
			if (hWindow)
			{
				if (DBGetContactSettingByte(NULL, "Chat", "ToggleVisibility", 0)==1 && !CallService(MS_CLIST_GETEVENT, (WPARAM)hContact, 0))
				{
...
HANDLE CList_FindRoom (char * pszModule, char * pszRoom) 
{
	...
	{
		szProto = (char *) CallService(MS_PROTO_GETCONTACTBASEPROTO, (WPARAM) hContact, 0);
		if (szProto && !lstrcmpi(szProto, pszModule)) 
		{
			if(DBGetContactSettingByte(hContact, szProto, "ChatRoom", 0) != 0)
			{
				if (!DBGetContactSetting(hContact, szProto, "ChatRoomID", &dbv) && dbv.type == DBVT_ASCIIZ) 
				{
					if(!lstrcmpi(dbv.pszVal, pszRoom))
					{
						DBFreeVariant(&dbv);
						return hContact;
					}
