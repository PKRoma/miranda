...
void  p2p_sendAck( filetransfer* ft, ThreadData* info, P2P_Header* hdrdata )
{
	...
	if ( ft == NULL ) {
		...
		if ( MSN_GetStaticString( "e-mail", info->mJoinedContacts[0], email, MSN_MAX_EMAIL_LEN ))
			return;
		sprintf( buf, sttP2Pheader, email );
	}
	else
	...
	size_t			szContLen )
{
	...
	switch ( iKind ) {
		case -1:  ft->p2p_byemsgid  = tHdr->mID; break;
		case 500: ft->p2p_byemsgid  = tHdr->mID; break;
		case 603: ft->p2p_byemsgid  = tHdr->mID; break;
	}
	...
	const char*		msgbody )
{
	...
	ft->p2p_type = dwAppID == MSN_APPID_AVATAR2 ? MSN_APPID_AVATAR : dwAppID;
	ft->p2p_ackID = ft->p2p_type * 1000;
	...
	if ( dwAppID == MSN_APPID_AVATAR2 )
		dwAppID = MSN_APPID_AVATAR;

	if ( dwAppID == MSN_APPID_AVATAR && !strcmp( szEufGuid, "{A4268EEC-FEC5-49E5-95C3-F126696BDBF6}" )) {
		DBVARIANT dbv;
	...
	if ( dwAppID == MSN_APPID_FILE && !strcmp( szEufGuid, "{5D3E02AB-6190-11D3-BBBB-00C04F795683}" )) {
