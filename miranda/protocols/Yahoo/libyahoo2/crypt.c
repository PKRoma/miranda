...
char *yahoo_crypt(char *key, char *salt)
{
	...
	/* Add the key string.  */
	mir_md5_append(&ctx, (md5_byte_t *)key, key_len);

	/* Because the SALT argument need not always have the salt prefix we
	   add it separately.  */
	mir_md5_append(&ctx, (md5_byte_t *)md5_salt_prefix, sizeof (md5_salt_prefix) - 1);

	/* The last part is the salt string.  This must be at most 8
	   characters and it ends at the first `$' character (for
	   compatibility which existing solutions).  */
	mir_md5_append(&ctx, (md5_byte_t *)salt, salt_len);
	...
	/* Add key.  */
	mir_md5_append(&alt_ctx, (md5_byte_t *)key, key_len);

	/* Add salt.  */
	mir_md5_append(&alt_ctx, (md5_byte_t *)salt, salt_len);
	...
	for (cnt = key_len; cnt > 0; cnt >>= 1)
		mir_md5_append(&ctx, (cnt & 1) != 0 ? alt_result : (md5_byte_t *)key, 1);
