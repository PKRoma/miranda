...
static void toc_peformerror(int e)
{
    ...
    switch (e) {
        ...
        case 982:
            ...
            break;
        case 983:
            mir_snprintf(buf, sizeof(buf),Translate("You have been connecting and disconnecting too frequently.  Wait 10 minutes and try again.  If you continue to try, you will need to wait even longer."));
			break;
        case 989:
...
int aim_toc_parse(char *buf, int len)
{
    ...
    if (tdt->state == STATE_FLAPON) {
		...
		mir_snprintf(snd, sizeof(snd), "toc2_login %s %d %s %s %s %s %s", "login.oscar.aol.com", 29999, aim_util_normalize(tdt->username),
    ...
    else if (!_strcmpi(c, "CHAT_IN_ENC")) {
        int groupid, whisper;
        char *szUser, *tmp;
