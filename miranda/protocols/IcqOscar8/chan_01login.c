...
void handleLoginChannel(unsigned char *buf, WORD datalen, serverthread_start_info *info)
{
	...
	if (isLoginServer)
	{
		...
		packTLVWord(&packet, 0x0017, 0x0005);               // Client major version
		packTLVWord(&packet, 0x0018, 0x0025);               // Client minor version
		packTLVWord(&packet, 0x0019, 0x0001);               // Client lesser version
		packTLVWord(&packet, 0x001a, 0x0e90);               // Client build number
		packTLVDWord(&packet, 0x0014, 0x00000055);          // Client distribution number
