...
BOOL CJabberProto::FtSend( HANDLE hConn, filetransfer* ft )
{
	...
	_tstat( ft->std.files[ ft->std.currentFileNumber ], &statbuf );	// file size in statbuf.st_size
	if (( fd = _topen( ft->std.files[ ft->std.currentFileNumber ], _O_BINARY|_O_RDONLY )) < 0 ) {
	...
	if (( sid = xmlGetAttrValue( siNode,  _T("id"))) != NULL &&
		...
		if (( featureNode = xmlGetChildByTag( siNode, "feature", "xmlns", _T(JABBER_FEAT_FEATURE_NEG))) != NULL &&
			...
			if ( optionNode != NULL ) {
				...
				ft->type = ftType;
				ft->std.totalFiles = 1;
				ft->std.currentFile = mir_tstrdup( filename );
