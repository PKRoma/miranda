...
static BOOL CALLBACK JabberBookmarksDlgProc( HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam )
{
	...
	case WM_INITDIALOG:
	{
		...
		if (hIml) {
			ImageList_AddIcon(hIml, (HICON)JCallService(MS_SKIN_LOADPROTOICON,(WPARAM)ppro->m_szModuleName,(LPARAM)ID_STATUS_ONLINE) );
		...
		lvCol.cx = DBGetContactSettingWord( NULL, ppro->m_szModuleName, "bookmarksWnd_cx0", 120 );
		lvCol.iSubItem = 0;
		ListView_InsertColumn( lv, 0, &lvCol );

		lvCol.pszText = TranslateT( "Room JID / URL" );
		lvCol.cx = DBGetContactSettingWord( NULL, ppro->m_szModuleName, "bookmarksWnd_cx1", 210 );
		lvCol.iSubItem = 1;
		ListView_InsertColumn( lv, 1, &lvCol );

		lvCol.pszText = TranslateT( "Nick" );
		lvCol.cx = DBGetContactSettingWord( NULL, ppro->m_szModuleName, "bookmarksWnd_cx2", 90 );
		lvCol.iSubItem = 2;
		...
		Utils_RestoreWindowPosition( hwndDlg, NULL, ppro->m_szModuleName, "bookmarksWnd_" );
		return TRUE;
	}
	...
	case WM_CLOSE:
		{
			...
			DBWriteContactSettingWord( NULL, ppro->m_szModuleName, "bookmarksWnd_cx0", lvc.cx );
			ListView_GetColumn( hwndList, 1, &lvc );
			DBWriteContactSettingWord( NULL, ppro->m_szModuleName, "bookmarksWnd_cx1", lvc.cx );
			ListView_GetColumn( hwndList, 2, &lvc );
			DBWriteContactSettingWord( NULL, ppro->m_szModuleName, "bookmarksWnd_cx2", lvc.cx );

			Utils_SaveWindowPosition( hwndDlg, NULL, ppro->m_szModuleName, "bookmarksWnd_" );
