...
static void JabberSearchReturnResults( HANDLE  id, void * pvUsersInfo /*LIST<void>* */, U_TCHAR_MAP * pmAllFields /*U_TCHAR_MAP*/ )
{
	...
	int nUsersFound = plUsersInfo->getCount();
	...
	for (int i=0; i<nAllCount; i++)
	{
		...
		if (var && ListOfNonEmptyFields.getIndex(var)<0) continue;
	...
	for (int i=0; i<nFieldCount; i++)   
	{
		...
		if (var) Results.pszFields[i]=pmAllFields->operator [](var);
	}
...
TCHAR * CopyKey(TCHAR * key) { return mir_tstrdup(key); }
////////////////////////////////////////////////////////////////////////////////
...
static void JabberIqResultAdvancedSearch( XmlNode *iqNode, void *userdata )
{
	...
	if (   (( id = JabberGetPacketID( iqNode )) == -1 ) 
		|| (( type=JabberXmlGetAttrValue( iqNode, "type" )) == NULL) )
	{
...
static void JabberSearchAddUrlToRecentCombo(HWND hwndDlg, TCHAR * szAddr)
{
	...
	if (lResult==-1) SendDlgItemMessage(hwndDlg,IDC_SERVER,CB_ADDSTRING,0,(LPARAM)szAddr);
	...
	case WM_USER+10:
		{			
			...
			if (MyDat)	{
				...
				if(MyDat->Label) mir_free(MyDat->Label);
				if(MyDat->Var) mir_free(MyDat->Var);
				if(MyDat->defValue) mir_free(MyDat->defValue);
				free(MyDat);
			}
			else	
				JabberSearchRefreshFrameScroll(hwndDlg,dat);
...
{
	if ( !jabberOnline || !lParam) return 0;	//error
	HWND hwndDlg=(HWND) lParam;
	JabberSearchData * dat=(JabberSearchData *)GetWindowLong(hwndDlg,GWL_USERDATA);
	if ( !dat ) return 0; //error
