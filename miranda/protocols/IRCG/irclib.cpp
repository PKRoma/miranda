...
CIrcMessage::CIrcMessage(const TCHAR* lpszCmdLine, bool bIncoming, bool bNotify)
		...
		while( *pStart )
		{
			...
			if( *pStart )
			{
				if(bMbotInstalled && prefs->ScriptingEnabled)
				{
					...
					char * pszTemp = NULL;
					pszTemp = (char *) mmi.mmi_malloc(lstrlenA(pStart) + 1); 
					lstrcpynA(pszTemp, pStart, lstrlenA (pStart) + 1);
					...
					if(	Scripting_TriggerMSPRawIn(&pszTemp) && pszTemp)
					{
						...
						TCHAR* p = mir_a2t( pszTemp );  //!!!!
						CIrcMessage msg( p, true );
						Notify( &msg );
						mir_free( p );
					}
