...
static int DrawTextUtf(HDC hDC, unsigned char *text, LPRECT lpRect, UINT uFormat, LPSIZE lpSize)
{
  int res;
...
static void PaintItemSetting(HDC hdc,RECT *rc,int i,UINT itemState)
{
  ...
  if (setting[i].value==0 && !(setting[i].displayType&LIF_ZEROISVALID))
  {
    ...
    if (setting[i].displayType & LIF_CHANGEONLY)
      text = ICQTranslateUtfStatic(LPGENUTF("<unremovable once applied>"), str, MAX_PATH);
    else
      text = ICQTranslateUtfStatic(LPGENUTF("<empty>"), str, MAX_PATH);
  }
  else 
  {
    switch (setting[i].displayType & LIM_TYPE)
    {
      ...
      case LI_LONGSTRING:
      {
        text = (unsigned char*)BinaryToEscapes((char*)setting[i].value);
        alloced = 1;
        break;
      }
      ...
      case LI_NUMBER:
        text = str;
        itoa(setting[i].value, (char*)text, 10);
        break;

      case LI_LIST:
        if (setting[i].dbType == DBVT_ASCIIZ) 
        {
          text = ICQTranslateUtfStatic((unsigned char*)setting[i].value, str, MAX_PATH);
        }
        else 
        {
          int j;

          text = ICQTranslateUtfStatic(LPGENUTF("Unknown value"), str, MAX_PATH);
        ...
        case 0:
          switch (((LPNMHDR)lParam)->code) 
          {
            ...
            case PSN_APPLY:
              if(ChangesMade()) 
              {
                if (IDYES!=MessageBoxUtf(hwndDlg, LPGENUTF("You've made some changes to your ICQ details but it has not been saved to the server. Are you sure you want to close this dialog?"), LPGENUTF("Change ICQ Details"), MB_YESNOCANCEL))
                {
              ...
              switch(cd->nmcd.dwDrawStage)
              {
                ...
                case CDDS_ITEMPREPAINT:
                {
                  ...
                  if (setting[cd->nmcd.lItemlParam].displayType == LI_DIVIDER)
                  {
                    ...
                    SIZE textSize;
                    unsigned char str[MAX_PATH];
                    unsigned char *szText = ICQTranslateUtfStatic(setting[cd->nmcd.lItemlParam].szDescription, str, MAX_PATH);
                    HFONT hoFont;
...
*/
      ...
      switch(LOWORD(wParam)) 
      {
        ...
        case IDC_SAVE:
          ...
          EnableDlgItem(hwndDlg, IDC_LIST, FALSE);
          {
            ...
            SetDlgItemTextUtf(hwndDlg, IDC_UPLOADING, ICQTranslateUtfStatic(LPGENUTF("Upload in progress..."), str, MAX_PATH));
          }
    ...
    case DM_PROTOACK:
    {
      ...
      if (ack->result == ACKRESULT_SUCCESS)
      {
        ...
        null_snprintf(buf, sizeof(buf), (unsigned char*)"%s%d%%", ICQTranslateUtfStatic(LPGENUTF("Upload in progress..."), str, MAX_PATH), 100*done/(SIZEOF(hUpload)));
        ...
        SetDlgItemTextUtf(hwndDlg, IDC_UPLOADING, ICQTranslateUtfStatic(LPGENUTF("Upload complete"), str, MAX_PATH));
      ...
      else if (ack->result==ACKRESULT_FAILED)
      {
        ...
        EnableDlgItem(hwndDlg, IDC_LIST, TRUE);
        EnableDlgItem(hwndDlg, IDC_UPLOADING, FALSE);
        SetDlgItemTextUtf(hwndDlg, IDC_UPLOADING, ICQTranslateUtfStatic(LPGENUTF("Upload FAILED"), str, MAX_PATH));
        SendMessage(GetParent(hwndDlg), PSM_FORCECHANGED, 0, 0);
      }
      break;
    }
