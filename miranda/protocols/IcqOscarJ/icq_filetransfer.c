...
static void file_sendNextFile(directconnect* dc)
{
	...
	if (dc->ft->iCurrentFile >= (int)dc->ft->dwFileCount)
	{
		...
		ProtoBroadcastAck(gpszICQProtoName, dc->ft->hContact, ACKTYPE_FILE, ACKRESULT_SUCCESS, dc->ft, 0);
		return;
	}
	...
	packLEWord(&packet, (WORD)(strlen(szThisSubDir) + 1));
	packBuffer(&packet, szThisSubDir, (WORD)(strlen(szThisSubDir) + 1));
