...
void InitDisplayNameCache(void)
{
	...
	displayNameCache=(struct displayNameCacheEntry*)realloc(NULL,displayNameCacheSize*sizeof(struct displayNameCacheEntry));
...
void FreeDisplayNameCache(void)
{
	...
	for(i=0;i<displayNameCacheSize;i++)
		free(displayNameCache[i].name);
	free(displayNameCache);
...
static int GetDisplayNameCacheEntry(HANDLE hContact)
{
	...
	for(i=0;i<displayNameCacheSize;i++) {
		if(hContact==displayNameCache[i].hContact) return i;
		if(firstUnused==-1 && displayNameCache[i].hContact==INVALID_HANDLE_VALUE) firstUnused=i;
	}
	if(firstUnused==-1) {
		firstUnused=displayNameCacheSize++;
		displayNameCache=(struct displayNameCacheEntry*)realloc(displayNameCache,displayNameCacheSize*sizeof(struct displayNameCacheEntry));
	}
...
int GetContactDisplayName(WPARAM wParam,LPARAM lParam)
{
	...
	if (!CallService(MS_CONTACT_GETCONTACTINFO,0,(LPARAM)&ci)) {
		if (ci.type==CNFT_ASCIIZ) {
			if (cacheEntry==-1) {
				buffer = (char*)malloc(strlen(ci.pszVal)+1);
	...
	//buffer = (char*)malloc(strlen(Translate("'(Unknown Contact)'"))+1);
