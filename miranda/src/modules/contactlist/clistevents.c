...
static int AddEvent(WPARAM wParam,LPARAM lParam)
{
	...
	event[i].cle.pszService=_strdup(event[i].cle.pszService);
	event[i].cle.pszTooltip=_strdup(event[i].cle.pszTooltip);
	eventCount++;
	if(eventCount==1) {
		char *szProto;
		if(event[0].cle.hContact==NULL) szProto=NULL;
...
static int RemoveEvent(WPARAM wParam, LPARAM lParam)
{
	...
	ChangeContactIcon(event[i].cle.hContact, IconFromStatusMode(szProto, szProto == NULL ? ID_STATUS_OFFLINE : DBGetContactSettingWord(event[i].cle.hContact,szProto,"Status",ID_STATUS_OFFLINE)),0);
...
void UninitCListEvents(void)
{
	...
	for(i=0;i<eventCount;i++) {
		if(event[i].cle.pszService) free(event[i].cle.pszService);
		if(event[i].cle.pszTooltip) free(event[i].cle.pszTooltip);
	}
	if(event!=NULL) free(event);
	if(imlIcon!=NULL) free(imlIcon);
}