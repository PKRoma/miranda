...
int InitModuleNames(void)
{
	...
	while(ofsThis) {
		...
		moduleName=(struct ModuleName*)realloc(moduleName,sizeof(struct ModuleName)*(moduleNameCount+1));
		moduleName[moduleNameCount].ofs=ofsThis;
		moduleName[moduleNameCount].hash=dbmn->cbName;    //very very simple hash so far
		moduleName[moduleNameCount].name=(char*)malloc(dbmn->cbName+1);
...
{
	int i;
	for(i=0;i<moduleNameCount;i++) free(moduleName[i].name);
	if(moduleNameCount) free(moduleName);
}
...
DWORD GetModuleNameOfs(const char *szName)
{
	...
	moduleName[moduleNameCount].name=(char*)malloc(nameLen+1);
