		...
		if ( pCachedValue != NULL ) {
			...
			if ( pCachedValue->type == DBVT_ASCIIZ ) {
				if ( isStatic ) {
					...
					dbcgs->pValue->pszVal = (char*)malloc(strlen(pCachedValue->pszVal)+1);
				...
				switch(pBlob[0]) {
					...
					case DBVT_ASCIIZ:
						...
						if(isStatic) {
							...
							dbcgs->pValue->pszVal=(char*)malloc(1+*(PWORD)(pBlob+1));
					...
					case DBVT_BLOB:
						...
						if(isStatic) {
							...
							dbcgs->pValue->pbVal=(char*)malloc(*(PWORD)(pBlob+1));
...
static int FreeVariant(WPARAM wParam,LPARAM lParam)
{
	...
	switch ( dbv->type ) {
		case DBVT_ASCIIZ:
		{
			if ( dbv->pszVal ) free(dbv->pszVal);
			dbv->pszVal=0;
			break;
		}
		case DBVT_BLOB:
		{
			if ( dbv->pbVal ) free(dbv->pbVal);
			dbv->pbVal=0;
			break;
		}
	}
