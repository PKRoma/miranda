...
*/
			if ( nlc->pHttpProxyPacketQueue == 0 && nlc->nlu->user.pfnHttpGatewayWrapSend != NULL )
				nlc->nlu->user.pfnHttpGatewayWrapSend((HANDLE)nlc,(PBYTE)"",0,MSG_NOHTTPGATEWAYWRAP,NetlibSend);
	...
	for( retryCount = 0;;) {
		...
		if(nlc->nlu->user.pfnHttpGatewayUnwrapRecv && !(flags&MSG_NOHTTPGATEWAYWRAP)) {
			PBYTE newBuffer;
			newBuffer=nlc->nlu->user.pfnHttpGatewayUnwrapRecv(nlhrReply,(PBYTE)nlhrReply->pData,nlhrReply->dataLength,&nlhrReply->dataLength,mir_realloc);
			if(newBuffer==NULL) {
				NetlibHttpFreeRequestStruct(0,(LPARAM)nlhrReply);
				return SOCKET_ERROR;
			}
	...
	if(nlhrReply->dataLength<=len) {
		...
		if(flags&MSG_PEEK) {
			nlc->dataBuffer=(PBYTE)nlhrReply->pData;
	...
	if(!(flags&MSG_PEEK)) {
		...
		nlhrReply->pData=(char*)mir_realloc(nlhrReply->pData,nlhrReply->dataLength-len);
