...
static int ClcModulesLoaded(WPARAM wParam,LPARAM lParam) {
	...
	for(i=0;i<protoCount;i++) {
		...
		clcProto=(ClcProtoStatus*)realloc(clcProto,sizeof(ClcProtoStatus)*(hClcProtoCount+1));
...
{     
    ...
    switch (msg)
    {
		case WM_CREATE:
			...
			dat=(struct ClcData*)malloc(sizeof(struct ClcData));
		...
		{	struct ClcContact *contact=NULL;
			...
			else status=DBGetContactSettingWord((HANDLE)wParam,szProto,"Status",ID_STATUS_OFFLINE);
			...
			if((dat->dragStage&DRAGSTAGEM_STAGE)==DRAGSTAGE_ACTIVE) {
				...
				switch(target) {
					...
					{	struct ClcGroup *group;
						...
						if(group->parent) hNewCursor=LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_DROPUSER));
						break;
					}
				}
		...
		{	struct ClcContact *contact;
			...
			if(dat->selection!=-1 && hitFlags&(CLCHT_ONITEMICON|CLCHT_ONITEMCHECK|CLCHT_ONITEMLABEL)) {
				if(contact->type==CLCIT_GROUP) {
					hMenu=GetSubMenu(LoadMenu(GetModuleHandle(NULL),MAKEINTRESOURCE(IDR_CONTEXT)),2);
