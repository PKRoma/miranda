...
BOOL CALLBACK DlgProcIconsOpts(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (msg)
	{
		...
		{	PROTOCOLDESCRIPTOR *proto,**protoList;
			...
			dat=(struct IconsOptsData*)malloc(sizeof(struct IconsOptsData));
			SetWindowLong(hwndDlg,GWL_USERDATA,(LONG)dat);
			dat->mainIconPath=(char**)malloc(sizeof(char*)*sizeof(mainIcons)/sizeof(mainIcons[0]));
			dat->mainStatusPath=(char**)malloc(sizeof(char*)*sizeof(skinIconStatusToIdStatus)/sizeof(skinIconStatusToIdStatus[0]));
			...
			i=SendDlgItemMessage(hwndDlg,IDC_CATEGORYLIST,LB_ADDSTRING,0,(LPARAM)Translate("Main Icons"));
			SendDlgItemMessage(hwndDlg,IDC_CATEGORYLIST,LB_SETITEMDATA,i,0);
			i=SendDlgItemMessage(hwndDlg,IDC_CATEGORYLIST,LB_ADDSTRING,0,(LPARAM)Translate("Global Status Icons"));
			SendDlgItemMessage(hwndDlg,IDC_CATEGORYLIST,LB_SETITEMDATA,i,0);
			...
			dat->protoIcons=(struct ProtoIconsData*)malloc(sizeof(struct ProtoIconsData)*dat->protoCount);
		...
		{	LVITEM lvi;
			...
			if(proto==NULL) {
				if (SendDlgItemMessage(hwndDlg,IDC_CATEGORYLIST,LB_GETCURSEL,0,0)==0) {
					for(lvi.iItem=0;lvi.iItem<sizeof(mainIcons)/sizeof(mainIcons[0]);lvi.iItem++) {
					...
					for(lvi.iItem=0;lvi.iItem<sizeof(skinIconStatusToIdStatus)/sizeof(skinIconStatusToIdStatus[0]);lvi.iItem++) {
						lvi.pszText=(char*)CallService(MS_CLIST_GETSTATUSMODEDESCRIPTION,skinIconStatusToIdStatus[lvi.iItem],0);
		...
		case DM_CHANGEICON:
			{	char *path=(char*)lParam;
				...
				if(proto==NULL) {
					if (SendDlgItemMessage(hwndDlg,IDC_CATEGORYLIST,LB_GETCURSEL,0,0)==0) {
						if(dat->mainIconPath[wParam]!=NULL) free(dat->mainIconPath[wParam]);
						dat->mainIconPath[wParam]=_strdup(path);
					}
					else {
						if(dat->mainStatusPath[wParam]!=NULL) free(dat->mainStatusPath[wParam]);
						dat->mainStatusPath[wParam]=_strdup(path);
					}
				}
				else {
					LVITEM lvi;
					...
					if(dat->protoIcons[i].iconPath[lvi.lParam]!=NULL) free(dat->protoIcons[i].iconPath[lvi.lParam]);
					dat->protoIcons[i].iconPath[lvi.lParam]=_strdup(path);
				}
			}
			SendMessage(hwndDlg,DM_REBUILDICONSPREVIEW,0,0);
		...
		case DM_CHANGESPECIFICICON:
			{	struct IconPreview *ico=(struct IconPreview*)lParam;
				...
				if(proto==NULL) {
					...
					for(i=0;i<dat->protoCount;i++)
						if(proto==dat->protoIcons[i].proto) break;
					if(dat->protoIcons[i].iconPath[ico->id]!=NULL) free(dat->protoIcons[i].iconPath[ico->id]);
					dat->protoIcons[i].iconPath[ico->id]=_strdup(ico->description);
				}
			}
			SendMessage(GetParent(hwndDlg), PSM_CHANGED, 0, 0);
			break;
			...
			if(LOWORD(wParam)==IDC_IMPORT) {
				...
				*pfilter='\0';
				...
				if(GetOpenFileName(&ofn)) {
					...
					if(SendDlgItemMessage(hwndDlg,IDC_CATEGORYLIST,LB_GETCURSEL,0,0)==0) {
						for(i=0;i<sizeof(mainIcons)/sizeof(mainIcons[0]);i++) {
							hIcon=ExtractIcon(GetModuleHandle(NULL),filename,-(int)(mainIcons[i].id<SKINICON_OTHER_MIRANDA?eventIconToResourceId[mainIcons[i].id-SKINICON_EVENT_MESSAGE]:otherIconToResourceId[mainIcons[i].id-SKINICON_OTHER_MIRANDA]));
							if(hIcon==NULL) continue;
							ico.id=mainIcons[i].id;
							ico.main=1;
							wsprintf(path,"%s,%d",filename,-(int)(mainIcons[i].id<SKINICON_OTHER_MIRANDA?eventIconToResourceId[mainIcons[i].id-SKINICON_EVENT_MESSAGE]:otherIconToResourceId[mainIcons[i].id-SKINICON_OTHER_MIRANDA]));
						...
						for(i=0;i<sizeof(skinIconStatusToIdStatus)/sizeof(skinIconStatusToIdStatus[0]);i++) {
							hIcon=ExtractIcon(GetModuleHandle(NULL),filename,-(int)skinIconStatusToResourceId[i]);
							if(hIcon==NULL) continue;
							ico.id=i;
							ico.main=0;
							wsprintf(path,"%s,%d",filename,-(int)skinIconStatusToResourceId[i]);
							SendMessage(hwndDlg,DM_CHANGESPECIFICICON,(WPARAM)(PROTOCOLDESCRIPTOR**)NULL,(LPARAM)&ico);
						}
					}
					else {
						...
						case ID_RESET:
						{
							...
							if(proto==NULL) {
								...
								if(dat->protoIcons[i].iconPath[lvi.lParam]!=NULL) {
									free(dat->protoIcons[i].iconPath[lvi.lParam]);
...
{
	HICON hIcon;
	hIcon=ExtractIcon(hMiranda,filename,-id);
...
BOOL CALLBACK DlgProcIconIndex(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (msg)
	{
		case WM_INITDIALOG:
			...
			SetDlgItemText(hwndDlg,IDC_ICONSET,"icons.dll");
			return TRUE;
		case DM_REBUILDICONSPREVIEW:
		{	LVITEM lvi;
			...
			switch(LOWORD(wParam)) {
				...
				{	char str[MAX_PATH];
					...
					*pfilter='\0';
					...
					SetDlgItemText(hwndDlg,IDC_ICONSET,str);
					break;
				}
				case IDC_GETMORE:
				{	char szVer[64],szUrl[256];
				...
				{	char filetmp[MAX_PATH],filename[MAX_PATH],path[MAX_PATH];
					...
					GetDlgItemText(hwndDlg,IDC_ICONSET,filetmp,sizeof(filetmp));
                    CallService(MS_UTILS_PATHTORELATIVE, (WPARAM)filetmp, (LPARAM)filename);
					ico.description=path;
		...
		case WM_LBUTTONUP:
			if(dragging) {
				...
				if(dropHiLite!=-1) {
					...
					ListView_GetItem(GetDlgItem(hwndDlg,IDC_PREVIEW),&lvi);
					wsprintf(path,"%s,%d",filename,(int)lvi.lParam);
