...
tPaintCallbackProc StatusPaintCallbackProc(HWND hWnd, HDC hDC, RECT * rcPaint, HRGN rgn, DWORD dFlags, void * CallBackData)
{
    DrawBackGround(hwndStatus,hDC); 
    return 0;
}
void DrawBackGround(HWND hwnd, HDC mdc)
{
...
//	HBITMAP hBmpOsb;
    ...
    if (rcPaint->right-rcPaint->left<=0 || rcPaint->bottom-rcPaint->top<=0) 
    {
        if (mdc==NULL)
        {
    ...
    SkinDrawGlyph(hdc,&clRect,rcPaint,"Main,ID=StatusBar");
	...
	//call to draw icons
	{
        ...
        sameWidth=DBGetContactSettingByte(NULL,"CLUI","EqualSections",1);
	    startoffset=DBGetContactSettingDword(NULL,"StatusBar","FirstIconOffset",0);
		extraspace=DBGetContactSettingDword(NULL,"StatusBar","BkExtraSpace",1);
			...
			clrc.right-=startoffset*2;
            if (nParts)
			    sectwidth=clrc.right/nParts;
            else sectwidth=clrc.right;
