...
$Id$
$Log$
Revision 1.1  2001/02/11 00:22:15  tristan_vdv
Initial revision
...
void icq_SetProxy(ICQLINK *link, const char *phost, unsigned short pport, int pauth, const char *pname, const char *ppass)
{
  ...
  link->icq_UseProxy = 1;
  link->icq_ProxyHost = _strdup(phost);
  link->icq_ProxyPort = pport;
  link->icq_ProxyAuth = pauth;
  link->icq_ProxyName = _strdup(pname);
  link->icq_ProxyPass = _strdup(ppass);
}
