...
#define PM_START               10   // If the plugin needs to do init work, do it here instead of Load, do as little as possible in load
#define PM_VERSION				11	//tells the plugin what version of Miranda is running, lparam=pointer to str of miranda ver (ie "0.0.7"), this message should be the first notify msg recieved by the plugin

/*
STILL UNIMPLIMENTED
#define PM_MSN_STATUSCHANGE		12 //msn status has changed, wparam containts int status
*/
...
typedef struct
{
	...
	void (*pGetPluginStr)(char *plugin, long, char *val, char *name, char *def);
	// set int in reg
