...
#ifdef TCP_TRACE_ODS
{char str[128];
  sprintf(str,"file request packet received from %lu { sequence=%lx, message=%s }\n",
     uin, id, message);
...
#ifdef TCP_TRACE_ODS
{char str[128];
      sprintf(str,"file 01 packet sent to uin %lu\n", plink->remote_uin);
  OutputDebugString(str);}
#else
...
#ifdef TCP_TRACE_ODS
{char str[128];
      sprintf(str,"file 02 packet sent to uin %lu\n", plink->remote_uin);
  OutputDebugString(str);}
#else
...
#ifdef TCP_TRACE_ODS
{char str[128];
      sprintf(str,"file 03 packet sent to uin %lu\n", plink->remote_uin);
  OutputDebugString(str);}
#else
