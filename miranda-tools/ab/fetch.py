...
t = time.gmtime()
name = 'miranda_' + str(t[0]).zfill(4) + str(t[1]).zfill(2) + str(t[2]).zfill(2) # build the name using yyyymmdd format
fp.write('cd /home/groups/m/mi/miranda-icq/htdocs/\n')
fp.write('mkdir alpha\n')
fp.write('chmod g+w alpha\n');
fp.write('cd alpha\n')
fp.write('put zip/' + name + '.zip\n')
fp.write('put zip/' + name + '_debug.zip\n')
...
def generate_html_index(fp,rel,dbg,src):
	fp.write('<html><head><title>Miranda Automatic Builds</title></head>\n')
	fp.write('<pre>\n')
	fp.write('<a href="' + rel + '">' + rel + '</a> <br>\n')
	fp.write('<a href="' + dbg + '">' + dbg + '</a> <br>\n' )
	fp.write('<a href="' + src + '">' + src + '</a> <br>\n')
	fp.write('<a href="ChangeLog">ChangeLog</a> <br>\n')
	fp.write('<a href="http://www.python.org"><img border=0 src="PythonPowered.gif"></a> <br>\n')
	fp.write('<i>generated by ' + USER + '</i> \n')
	fp.write('</html>')

try:
	fp=open('index.php','w')
except:
	print "Problem: Failed to create index.php!"
	sys.exit(1)
	
generate_html_index(fp,name + '.zip', name + '_debug.zip', 'miranda_src.zip')
fp.close()
